...
from JaxGME.utils import get_value
...
def guided_modes(g_array: bd.ndarray,
                 eps_array: bd.ndarray,
                 d_array: bd.ndarray,
                 n_modes: int = 1,
                 step: float = 1e-3,
                 tol: float = 1e-4,
    ...
    for ig, g in enumerate(g_array):
        ...
        om_lb = g_val / bd.sqrt(eps_val[1:-1]).max()
        om_ub = g_val / bd.sqrt(max(eps_val[0], eps_val[-1]))
        ...
        if ig > 0:
            ...
            """
            compute_g = bd.abs(g - g_array[ig - 1]) > 1e-8

        if compute_g:
            if bd.__class__.__name__ == 'JaxBackend':

                #assume te polorization and nmodes=1
                (omegas, coeffs) = guided_mode_given_g_jax(g=g_val,
                                                       eps_array=eps_array,
                                                       d_array=d_array,
                                                       omega_lb=om_lb,
                                                       omega_ub=om_ub,
...
def guided_mode_given_g(g,
    ...
    if omega_lb is None:
        omega_lb = g / bd.sqrt(eps_val[1:-1].max())
    if omega_ub is None:
        omega_ub = g / bd.sqrt(max(eps_val[0], eps_val[-1]))

    omega_lb = omega_lb * (1 + tol)
    omega_ub = omega_ub * (1 - tol)

    # Making sure the bounds go all the way to omega_ub
    omega_bounds = bd.linspace(omega_lb, omega_ub, int((omega_ub - omega_lb) / step) + 1)
    stepSize = omega_bounds[1]-omega_bounds[0]
    ...
    D22real = lambda x, g, eps_array, d_array: bd.real(D22(x, g, eps_array, d_array,pol=pol))
    ...
    lbs = omega_bounds[bd.diff(bd.sign(D22s),append=D22s[-1]) != 0][:1]
...
#this is a highly optomized version that uses the jax backend and assumes te polorization
#and only having one guided mode
def guided_mode_given_g_jax(g,
                        eps_array,
                        d_array,
                        n_modes=1,
                        omega_lb=None,
    ...
    # D22real is used in the fsolve; D22test is vectorized and used on a test
    # array of omega-s to find sign flips
    D22s = D22_jit(omega_bounds,g,eps_val, d_val).real

    #get the first lower bound
    lb = omega_bounds[bd.diff(bd.sign(D22s),append=D22s[-1]) != 0][0]
    ub = lb+stepSize

    # Compute guided mode frequency
    omega = bd.fsolve_D22(D22_jit, lb, ub, g, eps_array, d_array)

    #compute chis
    chi_array = chi_jit(omega, g, eps_array)

    # Compute A-B coefficients
    AB = AB_matrices_jax(d_array, chi_array)

    # Normalize
    norm = normalization_coeff_jax(g, d_array, chi_array, AB)

    return ([omega], [AB / bd.sqrt(norm)])
...
#the is the alternate version of the D22 function that works with jax.jit for increased speed
#I am going to choose to only look at TE modes for 3 layer systems to increase speed and simplify
@jax.jit
def D22_jit(omega, g, eps_array, d_array):
    """
    ...
    chis1 = 1j * bd.sqrt(g**2 - eps_array[0] * omega**2)
    chis2 = bd.array(bd.sqrt(-g**2 + eps_array[1] * omega**2), dtype=bd.complex)
    chis3 = 1j * bd.sqrt(g**2 - eps_array[2] * omega**2)
...
def D22s_vec(omegas, g, eps_array, d_array, pol='TM'):
    ...
    def S_T_prod(mats, omegas, g, eps1, eps2, d):
        ...
        T11 = bd.exp(1j * chis1 * d)
        T22 = bd.exp(-1j * chis1 * d)

        T_dot_mats = bd.zeros(mats.shape, dtype=bd.complex)
        T_dot_mats[0::2, :] = mats[0::2, :] * T11[:, bd.newaxis]
        T_dot_mats[1::2, :] = mats[1::2, :] * T22[:, bd.newaxis]

        S_dot_T = bd.zeros(mats.shape, dtype=bd.complex)
        S_dot_T[0::2,
    ...
    if eps_array.size == 3:
        ...
        mats = bd.zeros((2 * N_oms, 2), dtype=bd.complex)
        mats[0::2, 0] = S11
        mats[1::2, 0] = S21
        mats[0::2, 1] = S12
        mats[1::2, 1] = S22
...
#highly optomized version that uses jax backend, assumes te polorization and 3 layers
@jax.jit
def AB_matrices_jax(d_array, chi_array):
    """
    Function to calculate A,B coeff
    Output: array of shape [M+1,2]
    """
    ...
    S12 = -chi_array[:-1] + chi_array[1:]
    S_matrices = 0.5 / chi_array[1:].reshape(-1,1,1) * \
        bd.array([[S11,S12],[S12,S11]]).transpose([2,0,1])
    
    Tval = 1j * chi_array[1] * d_array / 2
    T11 = bd.exp(Tval)
    T22 = bd.exp(-1*Tval)
    T_matrices = bd.array([[T11, bd.zeros(T11.shape)],
                           [bd.zeros(T11.shape), T22]]).transpose([2, 0, 1])
    ...
    B0 = 1
    AB0 = bd.array([[A0], [B0]])
    AB1 = bd.dot(T_matrices[0], bd.dot(S_matrices[0], AB0))
    AB2 = bd.dot(S_matrices[1], bd.dot(T_matrices[0], AB1))

    return bd.array([AB0,AB1,AB2],dtype=bd.complex)
...
#highly optomized for jax.jit. assumes Te polorization and len(eps_array)==3
@jax.jit
def normalization_coeff_jax(g,
                            d_array,
                            chi_array,
                            ABref):
    """
    ...
    term1 = (bd.abs(chi1)**2 + g**2) * \
        (bd.abs(Bs[0])**2) * J_alpha(chi1-bd.conj(chi1))
    
    term2 = (bd.abs(chi3)**2 + g**2) * \
        (bd.abs(As[-1])**2) * J_alpha(chi3-bd.conj(chi3))
    
    term3 = (bd.abs(chi2)**2 + g**2) * (
            (bd.abs(As[1:-1])**2 + bd.abs(Bs[1:-1])**2) * \
            I_alpha(chi2-bd.conj(chi2), d_array)) + \
            (g**2 - bd.abs(chi2)**2) * (
            (bd.conj(As[1:-1]) * Bs[1:-1] + As[1:-1] * bd.conj(Bs[1:-1])) *
            I_alpha(-chi2-bd.conj(chi2), d_array)  )
