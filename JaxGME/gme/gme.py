...
from JaxGME.print_backend import print_backend as prbd
from JaxGME.utils import get_value, ftinv, z_to_lind
from JaxGME.print_utils import verbose_print, load_bar
...
class GuidedModeExp(object):
    ...
    def __init__(self, phc, gmax=3., truncate_g='abs'):
        ...
        self.ind_modes = []
        # Total number of basis vectors (equal to bd.sum(self.modes_numg))
        self.N_basis = []
        ...
        self._square_an = bd.arange(-360, 405, 45)
        self._hex_an = bd.arange(-360, 390, 30)
        self._rec_an = bd.arange(-360, 450, 90)

        # Initialize the reciprocal lattice vectors and compute the FT of all
        # the layers of the PhC
        if self.truncate_g == 'abs':
            self._init_reciprocal_abs()
            self._compute_ft_abs()
        else:
            raise ValueError("'truncate_g' must be 'abs'.")
    ...
    def _init_reciprocal_abs(self):
        ...
        """
        n1max = bd.int_(
            (4 * bd.pi * self.gmax) / bd.norm(self.phc.lattice.b1))
        n2max = bd.int_(
            (4 * bd.pi * self.gmax) / bd.norm(self.phc.lattice.b2))

        inds1 = bd.tile(bd.arange(-n1max, n1max + 1), (2*n2max + 1, 1))  \
                         .reshape((2*n2max + 1)*(2*n1max + 1), order='F')
        inds2 = bd.tile(bd.arange(-n2max, n2max + 1), 2 * n1max + 1)

        # Add dimension to indexes arrays
        #inds1 = inds1[bd.newaxis, :]
        #inds2 = inds2[bd.newaxis, :]
        gvec = self.phc.lattice.b1[:, bd.newaxis].dot(inds1[bd.newaxis, :]) + \
                self.phc.lattice.b2[:, bd.newaxis].dot(inds2[bd.newaxis, :])
        gnorm = bd.sqrt(gvec[0, :]**2 + gvec[1, :]**2)

        # Avoid to cut with gmax equal to one of the |G|
        if self.gmax * 2 * bd.pi in gnorm:
            gmax = self.gmax
            while gmax * 2 * bd.pi in gnorm:
                gmax += 1e-10

            gvec = gvec[:, gnorm <= 2 * bd.pi * gmax]
            inds1 = inds1[gnorm <= 2 * bd.pi * gmax]
            inds2 = inds2[gnorm <= 2 * bd.pi * gmax]
            print(
                f"Warning: gmax={self.gmax} exactly equal to one of the g-vectors modulus"
                f", reciprocal lattice truncated with gmax={gmax}"
                f" to avoid problems."
                f"\nPlane waves used in the expansion = {bd.shape(gvec)[1]}.")
        else:
            gvec = gvec[:, gnorm <= 2 * bd.pi * self.gmax]
            inds1 = inds1[gnorm <= 2 * bd.pi * self.gmax]
            inds2 = inds2[gnorm <= 2 * bd.pi * self.gmax]
    ...
    def _construct_sym_mat(self, theta):
        ...
        """
        mat = bd.array([[
            bd.cos(2 * theta * bd.pi / 180),
            bd.sin(2 * theta * bd.pi / 180)
        ], [bd.sin(2 * theta * bd.pi / 180),
            -bd.cos(2 * theta * bd.pi / 180)]])
        return mat
    ...
    def _get_guided(self, gk, kind, mode):
        ...
        def interp_coeff(coeffs, il, ic, indmode, gs):
            ...
            c_interp = bd.interp(gk[indmode], gs, coeffs[:, il, ic, 0])
        ...
        def interp_guided(im, ik, omegas, coeffs):
            ...
            indmode = bd.argwhere(gk > gs[0] - 1e-10).ravel()
        ...
        if mode % 2 == 0:
            (indmode, oms, As, Bs,
             chis) = interp_guided(mode // 2, ik, self.omegas_te,
                                   bd.array(self.coeffs_te))
        else:
            (indmode, oms, As, Bs,
             chis) = interp_guided(mode // 2, ik, self.omegas_tm,
                                   bd.array(self.coeffs_tm))
        return (indmode, oms, As, Bs, chis)
    ...
    def _compute_guided(self, g_array):
        ...
        g_array = bd.argset(g_array,-1,g_array[-1]+self.delta_gabs)
        self.g_array.append(g_array)
        self.gmode_te = self.gmode_inds[bd.remainder(self.gmode_inds, 2) == 0]
        self.gmode_tm = self.gmode_inds[bd.remainder(self.gmode_inds, 2) != 0]
        ...
        if self.gmode_te.size > 0:
            (omegas_te,
             coeffs_te) = guided_modes(g_array,
                                       ...
                                       n_modes=1 + bd.amax(self.gmode_te) // 2,
                                       tol=self.gmode_tol,
                                       pol='TE')
        ...
        if self.gmode_tm.size > 0:
            (omegas_tm,
             coeffs_tm) = guided_modes(g_array,
                                       ...
                                       d_a,
                                       step=self.gmode_step,
                                       n_modes=1 + bd.amax(self.gmode_tm) // 2,
                                       tol=self.gmode_tol,
                                       pol='TM')
    ...
    def _compute_ft_abs(self):
        ...
        ggridx = (self.gvec[0, :][bd.newaxis, :] -
                  self.gvec[0, :][:, bd.newaxis]).ravel()
        ggridy = (self.gvec[1, :][bd.newaxis, :] -
                  self.gvec[1, :][:, bd.newaxis]).ravel()

        # The dimension of G1 is [2,Ng^2]
        self.G1 = bd.vstack((ggridx, ggridy))

        # indexes corresponding to ggdrix,ggridy
        indgridx = (self.inds1[bd.newaxis, :] -
                    self.inds1[:, bd.newaxis]).ravel().astype(int)
        indgridy = (self.inds2[bd.newaxis, :] -
                    self.inds2[:, bd.newaxis]).ravel().astype(int)
        ...
        num_unique = bd.shape(unique)[1]

        # Unique g-vectors for calculting f-transform
        gvec_unique = self.phc.lattice.b1[:, bd.newaxis].dot(unique[0][bd.newaxis, :]) + \
                        self.phc.lattice.b2[:, bd.newaxis].dot(unique[1][bd.newaxis, :])
        ...
        for layer in layers:
            ...
            eps_ft = layer.compute_ft(bd.vstack((ggridx, ggridy))) 
            """
            """
    ...
    def _construct_mat(self, kind):
        ...
        gk = bd.sqrt(bd.square(gkx) + bd.square(gky))
        ...
        pp = bd.outer(pkx, pkx) + bd.outer(pky, pky)
        pq = bd.outer(pkx, qkx) + bd.outer(pky, qky)
        qq = bd.outer(qkx, qkx) + bd.outer(qky, qky)
        ...
        if gmode_include == []:
            ...
            self.gmode_include.append(bd.array(gmode_include))
        ...
        self.N_basis.append(bd.sum(bd.array(modes_numg)))
        ...
        """
        #print(bd.round(bd.triu(mat, 1) + bd.transpose(bd.conj(bd.triu(mat, 1))) + bd.real(bd.diag(bd.diag(mat))),3))
        #print(f'Dimension = {bd.shape(bd.round(bd.triu(mat, 1) + bd.transpose(bd.conj(bd.triu(mat, 1))) + bd.real(bd.diag(bd.diag(mat))),3))}')
            ...
            if self.truncate_g == 'tbt':
                ...
                for it, T1 in enumerate(self.T1):
                    ...
                    if bd.sum(bd.abs(
                        ...
                        self.hom_layer.append(True)
                        self.eps_mat.append(
                            bd.eye(bd.shape(eps_ft)[0],
                                   bd.shape(eps_ft)[1]) * T1[index_G0])
                        self.eps_inv_mat.append(
                            bd.eye(bd.shape(eps_ft)[0],
                                   bd.shape(eps_ft)[1]) / T1[index_G0])
                    else:
    ...
    def set_run_options(self,
        ...
        """Set multiple options for the guided-mode expansion.
            ...
            gmode_compute : {'exact', 'interp'}, optional
                ...
                interpolated ('interp').
            gmode_inds : list or bd.ndarray, optional
        ...
        for (option, value) in self._run_options.items():
            ...
            if option.lower() == 'gmode_inds':
                value = bd.array(value)
            # Set all the options as class attributes
            setattr(self, option, value)

    def run(self,
            kpoints: bd.ndarray = bd.array([[0], [0]]),
            angles: bd.ndarray = bd.array([]),
            **kwargs):
        """
        ...
        **kwargs
        ...
        # A separate array where the values are converted from ArrayBox to numpy
        # array, if using the 'autograd' backend.
        # d_array_val = bd.array(list(get_value(layer.d) for layer in \
        #     self.phc.layers), dtype=bd.float).ravel()
        #----disabled autograd backedn
        d_array_val = []
        ...
        if self.gmode_compute.lower() == 'interp':
            t = time.time()
            kmax = bd.amax(
                bd.sqrt(bd.square(kpoints[0, :]) + bd.square(kpoints[1, :])))
            Gmax = bd.amax(
                bd.sqrt(
                    bd.square(self.gvec[0, :]) + bd.square(self.gvec[1, :])))

            # Array of g-points over which the guided modes will be computed
            g_array = bd.linspace(0, Gmax + kmax, self.gmode_npts)
            self._compute_guided(g_array)
            self.t_guided = time.time() - t
        ...
        for ik, k in enumerate(kpoints.T):
            ...
            if self.eig_solver == 'eigh':
                ...
                # Diagonalise matrix
                if self.kz_symmetry is None:
                    # NB: we shift the matrix by bd.eye to avoid problems at the zero-
                    # frequency mode at Gamma
                    (freq2, evecs) = bd.eigh(mat + bd.eye(mat.shape[0]))
                    freq1 = bd.sqrt(
                        bd.abs(freq2 - bd.ones(mat.shape[0]))) / 2 / bd.pi

                    i_near = bd.find_nearest(get_value(freq1), self.eig_sigma,
                                          self.numeig)
                    ...
                    freq1_odd = bd.sqrt(
                        bd.abs(freq2_odd -
                               bd.ones(mat_odd.shape[0]))) / 2 / bd.pi
                    zeros_arr = bd.zeros(
                        (self.even_counts[ik], bd.shape(evecs_odd)[1]))
                    ...
                    freq1_even = bd.sqrt(
                        bd.abs(freq2_even -
                               bd.ones(mat_even.shape[0]))) / 2 / bd.pi
                    zeros_arr = bd.zeros(
                        (self.odd_counts[ik], bd.shape(evecs_even)[1]))
                    evecs_even = bd.concatenate((evecs_even, zeros_arr))

                    symm1 = bd.concatenate((bd.full(self.even_counts[ik],
                                                    1,
                                                    dtype=int),
                                            bd.full(self.even_counts[ik],
                                                    -1,
                                                    dtype=int)))
                    ...
                    freq1 = bd.sqrt(
                        bd.abs(freq2 - bd.ones(mat_odd.shape[0]))) / 2 / bd.pi
                    ...
                    #Rewrite eigenvector in original basis
                    zeros_arr = bd.zeros(
                        (self.even_counts[ik], bd.shape(evec)[1]))
                    ...
                    freq1 = bd.sqrt(
                        bd.abs(freq2 - bd.ones(mat_even.shape[0]))) / 2 / bd.pi
                    ...
                    #Rewrite eigenvector in original basis
                    zeros_arr = bd.zeros(
                        (self.odd_counts[ik], bd.shape(evec)[1]))
                ...
                (freq2,
                 evecs) = bd.eigsh(mat + bd.eye(mat.shape[0]),
                                   k=self.numeig,
                                   sigma=(self.eig_sigma * 2 * bd.pi)**2 + 1)
                freq1 = bd.sqrt(
                    bd.abs(freq2 - bd.ones(self.numeig))) / 2 / bd.pi
        ...
        if self.kz_symmetry:
            if self.kz_symmetry.lower() == 'odd':
                kz_symms = -bd.ones_like(bd.array(freqs))
            elif self.kz_symmetry.lower() == 'even':
                kz_symms = bd.ones_like(bd.array(freqs))
    ...
    def run_im(self):
        ...
        minds = bd.arange(0, self.numeig)

        if len(self.freqs) == 0:
            raise RuntimeError("Run the GME computation first!")
        num_k = bd.shape(self.freqs)[0]
        ...
        unbalance_i[bd.abs(bd.array(freqs_i)) < 1e-15] = 0.5
    ...
    def _separate_hamiltonian_dense(self, mat, symm_mat, ik):
        ...
        for ind_alpha, alpha in enumerate(self.gmode_inds):
            ...
            gvec_used_ex = bd.zeros(bd.shape(gvec_used))
            ...
            if bd.remainder(alpha, 2) == 0:
                blocks.append(block)
                blocks_w.append(-block_w)
            elif bd.remainder(alpha, 2) != 0:
                blocks.append(block)
                blocks_w.append(block_w)
        ...
        if bd.size(mat_odd) != 0 and bd.size(mat_even) != 0:
            max_out_1 = bd.max(bd.abs(out_diag_1))
            max_out_2 = bd.max(bd.abs(out_diag_2))
        else:
            max_out_1 = 0
            max_out_2 = 0
    ...
    def _separate_hamiltonian_sparse(self, mat, symm_mat, ik):
        ...
        data_P = bd.ones(len(indexes_sigma))
        col_P = bd.arange(len(indexes_sigma))
        row_P = bd.array(indexes_sigma)
    ...
    def _compute_rad_components(self, kind: int, minds: list = [0]):
        ...
        if len(freqs) == 0:
            raise RuntimeError("Run the GME computation first!")
        if bd.max(bd.array(minds)) > self.numeig - 1:
        ...
        for im in minds:
                ...
                for clad_ind in [0, 1]:
                    ...
                    if mode1 % 2 == 0:
                        ...
                        rad = matrix_elements.mat_tm_te(
                            ...
                            Ys['te'][clad_ind], Xs['te'][clad_ind],
                            chis['te'][clad_ind], pq)
                    #print(f"kind={kind}, {im}, {indmode1.shape},{bd.shape(evec)}, {self.modes_numg[kind][im1]}")
                    rad = rad * bd.conj(
                        evec[count:count +
                             self.modes_numg[kind][im1]][:, bd.newaxis])
            ...
            for pol in ['te', 'tm']:
                ...
                c_l[pol] = bd.sqrt(bd.pi * rad_dos[0]) * rad_c[pol][0]
                c_u[pol] = bd.sqrt(bd.pi * rad_dos[1]) * rad_c[pol][1]
    ...
    def compute_rad(self, kind: int, minds: list = [0]):
        ...
        freqs_im = bd.array(rad_tot) / 2 / bd.pi
    ...
    def compute_rad_sp(self, kind: int, minds: list = [0]):
        ...
        # Compute radiation rate in units of frequency
        freqs_im = bd.array(rad_tot) / 2 / bd.pi
        freqs_im_te = bd.array(rad_tot_te) / 2 / bd.pi
        freqs_im_tm = bd.array(rad_tot_tm) / 2 / bd.pi
    ...
    def get_eps_xy(self, z: float, xgrid=None, ygrid=None, Nx=100, Ny=100):
        ...
        xgrid : None, optional
            None or a 1D bd.array defining a grid in x.
        ygrid : None, optional
            None or a 1D bd.array defining a grid in y.
        Nx : int, optional
        ...
        if self.truncate_g == "tbt":
            ft_coeffs = bd.hstack(
                (self.T1[lind], self.T2[lind], bd.conj(self.T1[lind]),
                 bd.conj(self.T2[lind])))
    ...
    def ft_field_xy(self, field, kind, mind, z):
        ...
        omega = self.freqs[kind][mind] * 2 * bd.pi
    ...
    def get_field_xz(self,
        ...
        xgrid : None, optional
            None or a 1D bd.array defining a grid in x.
        zgrid : None, optional
            None or a 1D bd.array defining a grid in z.
        component : str, optional
            A string containing 'x', 'y', and/or 'z'
        Nx : int, optional
            If xgrid==None, a grid of Nx points in the elementary cell is 
