...
class Shape(object):
    ...
    def _parse_ft_gvec(self, gvec):
        if type(gvec) == list:
            gvec = bd.array(gvec)
        elif not isinstance(gvec,bd.ndarray):
...
class Circle(Shape):
    ...
    def __init__(self, eps=1., x_cent=0., y_cent=0., r=0.):
        ...
        self.area = bd.array([bd.pi*self.r**2])
    ...
    def compute_ft(self, gvec):
        ...
        gabs = bd.sqrt(bd.abs(bd.square(gx)) + bd.abs(bd.square(gy)))
        gabs += 1e-5  # To avoid numerical instability at zero
        ft = bd.exp(-1j*gx*self.x_cent - 1j*gy*self.y_cent)*self.r* \
                            2*bd.pi/gabs*bd.bessel1(gabs*self.r)
        
        return ft

    def is_inside(self, x, y):
        return (bd.square(x - self.x_cent) + bd.square(y - self.y_cent)
                <= bd.square(self.r))
...
class Ellipse(Shape):
    ...
    def is_inside(self, x, y):
        ...
        return (bd.square((x1 - self.x_cent) / self.rx) + bd.square(
...
class Poly(Shape):
    ...
    def __init__(self, eps=1., x_edges=[0.], y_edges=[0.]):
        ...
        eps : float
            Permittivity value
        x_edges : List or bd.ndarray
            x-coordinates of polygon vertices
        y_edges : List or bd.ndarray
    ...
    def compute_ft(self, gvec):
        ...
        ind_gx0 = bd.abs(gx[:, 0]) < 1e-10
        ind_gx = ~ind_gx0
        if bd.sum(ind_gx0) > 0:
            # And first the Gy = 0 case
            ind_gy0 = bd.abs(gy[:, 0]) < 1e-10
            if bd.sum(ind_gy0 * ind_gx0) > 0:
    ...
    def is_inside(self, x, y):
        vert = bd.vstack((bd.array(self.x_edges), bd.array(self.y_edges)))
        path = mpltPath.Path(vert.T)
        points = bd.vstack((bd.array(x).ravel(), bd.array(y).ravel()))
...
class Hexagon(Poly):
    ...
    def __init__(self, eps=1, x_cent=0, y_cent=0, a=0):
        ...
        y_edges = y_cent + bd.array([0, bd.sqrt(3)/2*a, bd.sqrt(3)/2*a, 0, \
                    -bd.sqrt(3)/2*a, -bd.sqrt(3)/2*a, 0])
        super().__init__(eps, x_edges, y_edges)
...
class FourierShape(Poly):
    ...
    def __init__(self,
        ...
        phis = bd.linspace(0, 2 * bd.pi, npts + 1)
